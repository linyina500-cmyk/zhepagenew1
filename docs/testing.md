...
| GitHub Actions 中的 Playwright | Chromium、Firefox、WebKit 加载生产构建与本地字体，使用界面完成导入、排版、图片、保存和 PNG 下载；检查真实 DOM 高度与正文顺序 | 所有浏览器/设备、第三方网站抓取稳定性、真实操作系统剪贴板格式和人工视觉审美 |
...
`manual-line-breaks.spec.ts` 检查普通回车、段末 Shift+Enter、首尾空段和粘贴空行在编辑器与预览中一致。`pagination-line-breaks.spec.ts` 将实际分页器与实际样式一起运行，核对跨页换行的顺序、行高与行内格式，避免同一次换行在新页重复占行。导入及自动排版前后的快速输入还通过真实 Tiptap 撤销/重做测试，确保这些操作各自独立。

同步测试使用本地模拟服务与测试账号，覆盖完整 PNG、独立文案、多账号勾选、浏览器加密存储、发布确认及结果恢复；不会向真实账号发布内容。真实公众号草稿验收与小红书登录验收单独记录，不能用模拟测试代替。
...
开发主机可用隔离的系统 Chrome 测试本地产物；这与操作用户日常浏览器分开。跨内核结果以相应提交的 Actions 运行记录为准，不能把先前提交的结果当作新改动的验证。
