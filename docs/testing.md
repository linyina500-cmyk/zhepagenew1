...
1. 用户长文通过富文本导入，保留五个 NBSP 空段，经过自动排版及美化后/基础样式切换，正文仍完整，七个章节仍存在。
...
每个浏览器保留 HTML 报告、失败 trace、失败截图及 PNG 导出附件 14 天。失败时先查看测试中的原文对照、页码及溢出高度，再查看 `workbench-status` 与 `uncaught-browser-errors` 附件。单次失败可重试一次；报告会保留首次失败的信息。修复应补入固定回归输入，不应仅增加重试或放大等待时间。
