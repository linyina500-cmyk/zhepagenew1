...
账号读取使用同一持久浏览器会话内的临时首页，结束后关闭临时页，保留原编辑器及其内容。每次关键账号核对均重新读取，不复用先前的成功结果。唯一可见账号卡 `.home-card-wrapper .personal .base .text` 内，`.account-name` 提供名称，`.others.description-text > div` 中严格以“小红书账号:”或“小红书账号：”开头的独立字段提供标识。该结构已于 2026-09-22 通过真实首页的可见 DOM 确认；不再依赖编辑器顶部不存在的用户主页链接。
...
- 账号：上述首页账号卡结构已核对，临时页读取与实际保存全过程仍需实机验收。没有唯一可见账号卡或明确标识时返回身份待确认，不要求已登录用户重复扫码。
