...
async function fixture(t, { resultStatus = "needs_confirmation", existingReceipt, switchedAccount = false } = {}) {
  ...
  const require = (specifier) => {
    ...
    if (specifier === "../../lib/wechat/client") return { createWechatClient: () => ({ getConnection: async () => ({ deviceId: "fixture-device" }) }) };
    if (specifier === "../../lib/wechat/deviceVault") return { loadBinding: async () => ({ deviceId: "fixture-device", connectionToken: "fixture-secret" }), saveBinding: async () => {} };
  ...
  const props = {
    draft, contentReady: true, contentChanged: false, busy: false,
...
test("XHS panel has local-service actions and preserves complete independent poster inputs", async (t) => {
  ...
  assert.doesNotMatch(f.container.textContent, /插件|扩展|Tampermonkey|安装脚本/);
...
test("XHS human acknowledgement requires the checkbox and never becomes saved", async (t) => {
  const f = await fixture(t);
  await f.click("检查小红书账号"); await f.click("同步到小红书草稿箱");
