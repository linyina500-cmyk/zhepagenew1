...
function config(overrides = {}) {
  const values = { WECHAT_APP_ID: first.appId, WECHAT_APP_SECRET: `'${first.appSecret}'`, WECHAT_ACCOUNT_NAME: '`测试名称 #一=二`', WECHAT_SYNC_TOKEN: '"private-token"', WECHAT_DATA_DIR: "'/Users/测试/含 空格/jobs'", WECHAT_HOST: "127.0.0.1", WECHAT_PORT: "8788", ...overrides };
...
test("config import rejects malformed, ambiguous, and executable-looking input", () => {
  const invalid = [
    config({ WECHAT_APP_SECRET: "'unclosed" }),
    ...
    config() + "\n__proto__=pollute",
    "export " + config().split("\n").slice(1).join("\n"),
    config().replace("WECHAT_APP_SECRET", "WECHAT_UNKNOWN"),
    config() + "\0",
  ];
