...
export async function GET(request: Request) {
  ...
  if (!isPublicImageUrl(source)) return NextResponse.json({ error: "无效图片地址" }, { status: 400 });
  try {
    const response = await fetchPublicImage(source);
    const contentType = response.headers.get("content-type") || "";
    if (!response.ok || !contentType.startsWith("image/")) return NextResponse.json({ error: "图片读取失败" }, { status: 502 });
    const buffer = await response.arrayBuffer();
    if (buffer.byteLength > MAX_IMAGE_BYTES) return NextResponse.json({ error: "图片过大" }, { status: 413 });
