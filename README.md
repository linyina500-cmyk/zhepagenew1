...
开发分支新增网页草稿同步，页面按“确认内容 → 选择账号 → 保存结果”引导操作。管理员部署私有云端服务后，用户直接在网页连接账号和同步，无需下载启动工具。平台授权加密后由当前浏览器保存，云端按次临时使用；可选择是否记住账号。公众号仍需管理员做首次配置，小红书保存后仍需到平台核对。使用方式与验证边界见 [DRAFT_SYNC.md](DRAFT_SYNC.md)，部署方式见 [cloud/DEPLOYMENT.md](cloud/DEPLOYMENT.md)。
