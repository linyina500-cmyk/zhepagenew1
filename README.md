...
开发分支新增草稿同步，页面按“确认内容 → 选择账号 → 保存结果”引导操作。Mac 用户下载启动包、解压后双击“启动折页.command”，工具会自动准备组件、打开本机页面并连接，无需输入命令或复制配对码。账号资料留在电脑上；公众号仍需管理员做首次配置，小红书保存后仍需到平台核对。Windows 暂无双击启动包。使用方式见 [DRAFT_SYNC.md](DRAFT_SYNC.md)。
