...
- 同步弹窗按小红书（1080×1440）或公众号（1080×1350）自动适配图片；标题超长就地提醒，风险提示分平台编辑并单独确认后重绘原来的最后一页，不额外增加页数
...
开发分支新增贴图同步：打开弹窗自动准备当前图片，公众号单账号默认选中、多账号可勾选，统一使用“同步到草稿箱”；小红书通过本机专用浏览器登录、保存并回读草稿。公众号凭据保存在当前浏览器的独立加密资料库，连接时直接进入本机服务内存，不经云端中转。Mac 助手随登录启动，意外退出后恢复，网页连接地址不随重启变化。当前方案用于开发预览，尚未自动部署到正式网站。本轮公众号两张真实 PNG 草稿已保存、官方回读像素一致且用户已看到；网页立即发布入口已关闭，代码保留；公众号同步后需在官方后台检查封面，必要时重新选择封面并保存。小红书新链路尚未完成真实验收。使用方法与验证边界见 [DRAFT_SYNC.md](DRAFT_SYNC.md)，启动步骤见 [本机使用说明](server/wechat/本机使用说明.md)，连接配置见 [服务说明](server/wechat/README.md)。
