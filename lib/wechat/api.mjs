...
const fieldName = /^[a-z0-9_]{1,64}$/iu;
const fieldNames = (value) => value && typeof value === "object" && !Array.isArray(value)
  ? Object.keys(value).filter((name) => fieldName.test(name)).slice(0, 64) : [];
const validCoverMediaId = (value) => validIdentifier(value)
  && ![...value].some((character) => character.charCodeAt(0) < 32 || character.charCodeAt(0) === 127)
  && !/^(?:[a-z][a-z0-9+.-]*:|\/\/)/iu.test(value);

export function normalizeDraftCoverSource(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const { articleFieldNames, coverFieldNames } = value;
  if (![articleFieldNames, coverFieldNames].every((names) => Array.isArray(names) && names.length <= 64
    && names.every((name) => typeof name === "string" && fieldName.test(name)))) return undefined;
  const thumbMediaId = validCoverMediaId(value.thumbMediaId) ? value.thumbMediaId : undefined;
  const firstImageMediaId = validCoverMediaId(value.firstImageMediaId) ? value.firstImageMediaId : undefined;
  return {
    ...(thumbMediaId ? { thumbMediaId } : {}),
    ...(firstImageMediaId ? { firstImageMediaId } : {}),
    ...(thumbMediaId && firstImageMediaId ? { isFirstImage: thumbMediaId === firstImageMediaId } : {}),
    articleFieldNames: [...articleFieldNames],
    coverFieldNames: [...coverFieldNames],
  };
}

const verificationFields = ["type", "title", "content", "imageCount", "imageOrder"];
export function normalizeDraftVerificationMismatches(value) {
  if (!Array.isArray(value) || value.length > verificationFields.length || new Set(value).size !== value.length
    || value.some((field) => !verificationFields.includes(field))) return undefined;
  return verificationFields.filter((field) => value.includes(field));
}

function verificationMessage(article, mismatches) {
  if (mismatches.length === 0) return "标题、配文和图片顺序已核对。";
  if (!article || typeof article !== "object" || Array.isArray(article)) {
    return "暂未读到完整的草稿详情。请在公众号后台核对，无需重复同步。";
  }
  if (mismatches.includes("type")) return "读取的草稿类型与本次贴图不一致。请在公众号后台核对，无需重复同步。";
  const labels = { title: "标题", content: "配文", imageCount: "图片数量", imageOrder: "图片或顺序" };
  const differences = mismatches.map((field) => labels[field]).join("、");
  return `草稿已存在，但${differences}与本次同步记录不一致。请在公众号后台核对，无需重复同步。`;
}
...
export function createWechatApi({ appId, appSecret, fetchImpl = fetch }) {
  ...
  return {
      ...
      const data = await request("/cgi-bin/draft/add", {
        ...
        json: {
          articles: [{
            ...
            // substitute a news thumbnail or invent a portrait crop ratio.
            // A square-only cover_info did not suppress the API's landscape
            // crop defaults in a real draft trial, so it is not a cover fix.
    ...
    async verifyDraft({ draftId, title, body, imageMediaIds }) {
      ...
      if (article?.article_type !== "newspic") verificationMismatches.push("type");
      // The web editor trims surrounding title whitespace when saving a cover.
      // Internal spaces, punctuation, body text and image order remain exact.
      if (typeof article?.title !== "string" || article.title.trim() !== title.trim()) verificationMismatches.push("title");
      if (article?.content !== body) verificationMismatches.push("content");
      if (!Array.isArray(images) || images.length !== imageMediaIds.length) verificationMismatches.push("imageCount");
      else if (!images.every((image, index) => image?.image_media_id === imageMediaIds[index])) verificationMismatches.push("imageOrder");
      const verified = verificationMismatches.length === 0;
      const coverInfo = normalizeDraftCoverInfo(article?.cover_info);
      const coverSource = article && typeof article === "object" && !Array.isArray(article)
        ? normalizeDraftCoverSource({
          thumbMediaId: article.thumb_media_id,
          firstImageMediaId: Array.isArray(images) ? images[0]?.image_media_id : undefined,
          articleFieldNames: fieldNames(article),
          coverFieldNames: fieldNames(article.cover_info),
        }) : undefined;
      return {
        verified,
        verificationMismatches,
        message: verificationMessage(article, verificationMismatches),
        ...(coverInfo ? { coverInfo } : {}),
