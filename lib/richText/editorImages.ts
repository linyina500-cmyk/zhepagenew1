...
export async function insertImageFiles(view: EditorView, files: File[], onNotice: Notice, position?: number): Promise<boolean> {
  ...
  try {
    ...
    const imageNodes = sources.map((src, index) => initialState.schema.nodes.image.create({ src, alt: files[index].name, style: IMAGE_STYLE }));
    const insertedNodes = new Set(imageNodes);
    const transaction = closeHistory(view.state.tr)
      .setSelection(selection)
      .replaceSelection(new Slice(Fragment.fromArray(imageNodes), 0, 0));
    ...
    transaction.setSelection(NodeSelection.create(transaction.doc, positions[positions.length - 1]));
    ...
    view.focus();
    onNotice(`已插入 ${files.length} 张图片，可继续调整尺寸、对齐、圆角和图注`, "success");
    return true;
  } catch (error) {
