...
export const onRequestPost = async (context: { request: Request }) => {
  try {
    ...
    if (!body.url || !isPublicHttpUrl(body.url)) return json({ error: "请输入有效的公开文章地址" }, 400);
    const { response, finalUrl } = await fetchPublicHtml(body.url);
    if (!response.ok) return json({ error: `文章站点返回 ${response.status}` }, 502);
    const contentType = response.headers.get("content-type") || "";
    if (!contentType.includes("text/html") && !contentType.includes("application/xhtml+xml")) return json({ error: "这个地址不是可识别的网页文章" }, 415);
    const html = await response.text();
    if (new TextEncoder().encode(html).byteLength > MAX_HTML_BYTES) return json({ error: "文章内容过大，请改用 HTML 粘贴" }, 413);
    return json({ html, finalUrl });
  } catch {
    return json({ error: "暂时无法读取该文章，请尝试粘贴 HTML" }, 502);
  }
};
