...
// Same-origin Pages bridge. The fixed-IP service owns WeChat credentials and
// durable job records; the browser only holds its separate connection password.
export async function onRequest({ request, env }: Context): Promise<Response> {
  const incoming = new URL(request.url);
  if (incoming.search) return json("接口不接收网址查询参数", 400);
  const validGet = /^\/api\/wechat\/(account|jobs\/[a-f0-9-]{36})$/i.test(incoming.pathname);
  const validPost = /^\/api\/wechat\/jobs(?:\/[a-f0-9-]{36}\/verify)?$/i.test(incoming.pathname);
  if (!((request.method === "GET" && validGet) || (request.method === "POST" && validPost))) return json("没有此草稿操作", 404);
  let upstream: URL;
  try {
  ...
  } catch { return json("公众号同步服务尚未配置，请先完成服务连接配置", 503); }
  const authorization = request.headers.get("Authorization") || "";
  if (!/^Bearer \S{32,256}$/.test(authorization)) return json("请填写公众号连接口令", 401);
